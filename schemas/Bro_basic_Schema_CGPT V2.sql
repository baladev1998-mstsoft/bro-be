...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'property_registration_status') THEN
    CREATE TYPE property_registration_status AS ENUM (
      'draft',            -- created but not submitted
      'pending_review',   -- submitted, awaiting approval
      'approved',         -- approved and can be published
...
CREATE TABLE IF NOT EXISTS properties (
  ...
  registration_status property_registration_status DEFAULT 'draft',
  registration_submitted_by uuid REFERENCES users(id) ON DELETE SET NULL,
  registration_submitted_at timestamptz,
  property_meta jsonb DEFAULT '{}'::jsonb
);
...
CREATE TABLE IF NOT EXISTS property_assignments (
  ...
  notes text,
  assigned_scope text, -- human-friendly note/label
  created_at timestamptz DEFAULT now(),
